...
int main(int, char**) {
    ...
    CUDA_CHECK_AND_EXIT(cudaGetDeviceProperties(&device_properties, current_device));
    std::string gpu_architecture_option =
        "--gpu-architecture=compute_" + std::to_string(device_properties.major * 10 + device_properties.minor);
