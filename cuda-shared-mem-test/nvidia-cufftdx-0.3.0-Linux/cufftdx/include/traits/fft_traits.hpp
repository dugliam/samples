...
namespace cufftdx {
    ...
    struct stride_of {
        ...
        static constexpr value_type value =
            detail::get_or_default_t<fft_operator::stride, Description, Stride<1>>::value;
        constexpr operator value_type() const noexcept { return value; }
    };

    template<class Description>
    constexpr unsigned int stride_of<Description>::value;
    ...
    // [TODO] How should we handle cases when direction was not set? Static assert may not be the best option.
    // Maybe we should introduce a wrapper for value which can be compared to "cufftdx::value_not_set"? Implement std::optional?
    // We can also introduce has_operator or can_infer/can_deduce?
    template<class Description>
    struct direction_of {
    private:
    ...
    // [TODO] Think about better name for this trait
